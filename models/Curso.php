...
class Curso
{
    ...
    /**
     * Cursos activos visibles en el catálogo del asesor (respeta acceso_asesores / lista permitida).
     *
     * @return array<int, array<string,mixed>>
     */
    public static function activosParaAsesor(PDO $pdo, string $cedulaAsesor): array
    {
        if ($cedulaAsesor === '' || !CursoAccesoAsesor::tieneMigracion($pdo)) {
            return self::activos($pdo);
        }

        $joinCoord = self::tieneCedulaCoordinador($pdo)
            ? ' LEFT JOIN usuarios u ON u.cedula = c.cedula_coordinador '
            : '';
        $selCoord = self::tieneCedulaCoordinador($pdo) ? ', u.nombre AS nombre_coordinador ' : '';

        $sql = "SELECT DISTINCT c.* {$selCoord}
                FROM cursos c
                {$joinCoord}
                LEFT JOIN curso_asesores_permitidos cap
                  ON cap.id_curso = c.id_cursos AND cap.cedula_asesor = :a
                LEFT JOIN capacitaciones_asignadas ca
                  ON ca.id_curso = c.id_cursos AND ca.cedula_asesor = :a2
                WHERE c.estado = 'activo'
                  AND (
                    c.acceso_asesores = 'publico'
                    OR cap.cedula_asesor IS NOT NULL
                    OR ca.id_asignacion IS NOT NULL
                  )
                ORDER BY c.id_cursos DESC";
        $st = $pdo->prepare($sql);
        $st->execute(['a' => $cedulaAsesor, 'a2' => $cedulaAsesor]);
        $rows = $st->fetchAll();
        if (!self::tieneCedulaCoordinador($pdo)) {
            foreach ($rows as &$r) {
                $r['nombre_coordinador'] = null;
